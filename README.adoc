...
The `notice.txt` and `license.txt` files from `${project.root}/src/main/resources` are included in binary and source JARs under `META-INF`.
...
Be sure to adapt the version number to the latest release version. The second and already last step of the setup is to activate the assembly and Antora plugin in the build section:
...
* Antora documentation sources need to be in the root project (`src/main/antora`).
...
* A nightly build can then use `clean (dependency:tree) deploy -Pantora` to publish reference documentation
...
* `distribute` - Creates Javadoc file distribution.
